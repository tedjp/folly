...
void CompressionTest::runSimpleTest(const DataHolder& dh) {
  if (!hasCodec(codecType_)) {
    return;
  }

  auto codec = getCodec(codecType_);
  const auto original = split(IOBuf::wrapBuffer(dh.data(uncompressedLength_)));
  const auto compressed = split(codec->compress(original.get()));
  if (!codec->needsUncompressedLength()) {
